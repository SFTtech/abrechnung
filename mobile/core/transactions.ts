...
export const positionStateByTransaction = atomFamily<TransactionPosition[], number>({
    ...
    default: transactionID => transactionID === null ? [] : (async () => filterPositions(await getTransactionsPositions(transactionID)))(),
...
export const transactionsInvolvingAccount = selectorFamily<Transaction[], accountIdentifierParam>({
    ...
    get: ({ groupID, accountID }) => ({ get }) => {
        ...
        return transactions
            .filter(t => t.debitor_shares.hasOwnProperty(accountID) || t.creditor_shares.hasOwnProperty(accountID) || involvedTransactionIDsThroughPositions.has(t.id));
    },
});