        ...
        deleteTransaction(transaction.group_id, transaction.id)
            .then(() => {
                pushLocalTransactionChanges(transaction.id)
                    .catch(err => {
                        notify({ text: `Error while syncing transaction state with server: ${err.toString()}` });
                    });
            })
            .catch(err => {
                    ...
                    right={props => (
                        <>
                            {transaction.has_local_changes && (
                                <MaterialIcons style={{marginRight: 8, marginTop: 4}} size={20} color={theme.colors.primary} name="sync-disabled" />
                            )}
                            <Text>{transaction.value.toFixed(2)}{transaction.currency_symbol}</Text>
                        </>
                    )}
